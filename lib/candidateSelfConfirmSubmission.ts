...
function structuredRequirementReason(fieldName: string, value: unknown) {
  ...
  if (fieldName === "workExperience") {
    const valid = rows.some((item) => {
      const row = item as Record<string, any>;
      return Boolean(
        rowValue(row, "company", "employer") &&
          rowValue(row, "title", "role") &&
          rowValue(row, "startDate", "start_date") &&
          (rowValue(row, "endDate", "end_date") || row.current === true),
      );
    });
    return valid
      ? ""
      : "At least one employment row needs employer, title, start date, and end date or Current.";
  }
  if (fieldName === "projectExperience") {
    const valid = rows.some((item) => {
      const row = item as Record<string, any>;
      return Boolean(
        rowValue(row, "name", "client") &&
          rowValue(row, "role", "title") &&
          rowValue(row, "startDate", "start_date") &&
          rowValue(row, "endDate", "end_date"),
      );
    });
    return valid
      ? ""
      : "At least one SAP project needs project/client, role, start date, and end date.";
  }
...
export function validateCandidateSelfConfirmSubmission(
  ...
  return {
    ...
    valid:
      submission.candidateConsent && fieldResults.every((item) => item.valid),
    riskLevel,
    validationReasons,
    trustImpact,
    fieldResults,
