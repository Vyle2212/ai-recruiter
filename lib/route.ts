...
function explicitModulesForJob(requiredModule: SapPrimaryModule) {
  // V34: BTP must use keyword search first, not strict module array filtering.
  // Talent Pool Search finds BTP through indexed search_text, while many candidate all_modules arrays
  // contain adjacent tokens or inconsistent labels. Strict overlaps can make the Match pool become 0.
  if (requiredModule === "BTP") return null;
...
async function searchIndexManual(args: {
  ...
  if (modules?.length && keyword === "BTP") query = query.overlaps("all_modules", modules);
  else if (keyword) query = query.ilike("search_text", `%${String(keyword).replace(/[%_]/g, "")}%`);
  else if (modules?.length) query = query.overlaps("all_modules", modules);
