...
export function nameRejectReason(value: string) {
  ...
  if (/^(Robot Framework|Power BI|SuccessFactors)$/i.test(v) || /\b(?:service\s*now|hp\s*alm|\balm\b|jira|oracle|excel)\b/i.test(v) || ((v.match(/,/g)||[]).length > 0 && /\b(?:service\s*now|sap|oracle|alm|jira|excel)\b/i.test(v))) return "skill_or_tool_name";
...
export function titleRejectReason(value: string, modules: string[]) {
  ...
  if (/^(Manager|Specialist|Lead|Analyst|Consultant|Project Lead|Project Manager|SAP Consultant)$/i.test(v)) return modules.length ? "generic_title_needs_review" : "generic_title_without_sap_context";
  if (/^(Product Manager|MIS Manager|Recruitment Consultant|Chiropractor)$/i.test(v)) return "non_sap_title";
  return "";
}
...
export function employerRejectReason(value: string, clientCompanies: string[] = []) {
  ...
  if (BAD_COMPANY_RE.test(v) || /@|\b(?:yahoo|gmail|hotmail|outlook)\.com\b|EMPLOYMENT HISTORY|PROFESSIONAL EXPERIENCES|Date Company Name Role|technology enablement solutions|\b(?:enhancement|consultation|responsibilities|implementation|project|duration|role|present)\b/i.test(v)) return "company_sentence_or_project_fragment";
  ...
  const words = v.split(/\s+/).filter(Boolean);
  const legal = /\b(?:Sdn\.?\s*Bhd|Pte\.?\s*Ltd|Pvt\.?\s*Ltd|Ltd\.?|Inc\.?|Corporation|Consulting|Technologies|Solutions|Bank|Berhad|Co\.?\s*,?\s*Ltd|GmbH|LLC|Systems)\b/i.test(v);
...
function titleModule(title: string) {
  ...
  if (/\bPI\s*\/\s*PO\b|\bPI-PO\b|\bPI\s+Consultant\b/i.test(title)) return "PI/PO";
  if (/\bSolution Manager\b|\bSolMan\b/i.test(title)) return "Solution Manager";
...
function alignPrimaryModule(title: string, primary: string, modules: string[], rawText: string) {
  ...
  if (/\bSAP\b/i.test(title) && modules.includes(fromTitle)) return fromTitle;
...
export function validateAiCandidateExtraction(raw: RawAiCandidateExtraction, candidate: AnyRecord, rawText: string, provider: string, fallbackParserUsed: boolean, providerMeta?: AiExtractionProviderMeta): ValidatedAiCandidateExtraction {
  ...
  const currentCompanyYearsExperience = Number(raw.employer.currentCompanyYearsExperience?.value || 0);
  const currentCompanyTenureText = clean(raw.employer.currentCompanyTenureText?.value);
  const previousCompany = clean(raw.employer.previousEmployer?.value);
  const previousCompanyStartDate = clean(raw.employer.previousCompanyStartDate?.value);
  const previousCompanyEndDate = clean(raw.employer.previousCompanyEndDate?.value);
  ...
  return {
    ...
    currentCompanyEndDate,
    currentCompanyYearsExperience,
    currentCompanyTenureText,

    previousEmployer: previousCompany,
    previousCompany,
    previousCompanyStartDate,
    ...
    validationStatus: reviewClassification,
    ...
    providerUsed: providerMeta?.providerUsed || raw.providerMeta?.providerUsed || providerMeta?.mode || raw.providerMeta?.mode || "fallback",
    model: providerMeta?.model || raw.providerMeta?.model || provider,
    modelUsed: providerMeta?.model || raw.providerMeta?.model || provider,
    cacheHit: Boolean(providerMeta?.cacheHit || raw.providerMeta?.cacheHit),
    fallbackParserUsed,
    fallbackReason: providerMeta?.fallbackReason || raw.providerMeta?.fallbackReason || "",
    openAiExtractionUsed: Boolean(providerMeta?.openAiExtractionUsed || raw.providerMeta?.openAiExtractionUsed),
    openAiRequestAttempted: Boolean(providerMeta?.openAiRequestAttempted || raw.providerMeta?.openAiRequestAttempted),
    openAiRequestSucceeded: Boolean(providerMeta?.openAiRequestSucceeded || raw.providerMeta?.openAiRequestSucceeded),
    openAiErrorType: providerMeta?.openAiErrorType || raw.providerMeta?.openAiErrorType || "",
    openAiErrorMessage: providerMeta?.openAiErrorMessage || raw.providerMeta?.openAiErrorMessage || "",
  };
}
