...
export type RecruiterJobsAuthorization =
  | {
      allowed: true;
      mode: "local_preview" | "authenticated_recruiter";
      actor: { id: string | null; role: string; label: string };
    }
...
export async function authorizeRecruiterJobsRead(): Promise<RecruiterJobsAuthorization> {
  ...
  if (
    !["admin", "qa", "recruiter_manager", "recruiter"].includes(
      String(profile.role),
    )
  )
  ...
  return {
    allowed: true,
    mode: "authenticated_recruiter",
    actor: {
      id: String(profile.id),
      role: String(profile.role),
      label: String(profile.full_name || user.email || "Recruiter"),
    },
  };
}
