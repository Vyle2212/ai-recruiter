...
> Latest database-surface checkpoint: [v108 complete table/view/RPC cutover](precision-v108-database-surface-cutover-20260923.md). A fresh production read-only audit confirms the authorization defect remains open. The manual cutover now covers all 25 exposed tables, two owner-privileged audit views and six public candidate/search RPCs with deny-by-default service-role-only access. It has not run. Exact-head authenticated acceptance, version-matched backup, controlled execution/readback and all data/OCR gates remain blockers (NO_GO).
