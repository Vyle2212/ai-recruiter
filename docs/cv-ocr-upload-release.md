...
> Latest database-surface checkpoint: [v108 complete table/view/RPC cutover](precision-v108-database-surface-cutover-20260923.md). A fresh read-only audit confirms the production authorization defect is still open and extends the manual artifact to two owner-privileged audit views plus six publicly executable candidate/search RPCs. The artifact remains unexecuted; backup, authenticated acceptance, controlled cutover/readback and all OCR/backfill gates remain required. Production is NO_GO.
