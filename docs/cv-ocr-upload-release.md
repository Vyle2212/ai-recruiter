...
> RTF source checkpoint: [v148 shared RTF parser](precision-v148-rtf-shared-parser-20260925.md). Across the same 905 originals, RTF support reduces source extraction failures from three to zero and adds ten valid employment rows without changing complete-for-validation totals. Fourteen PDFs still require OCR, 61 PDFs need layout fallback, and the collection has only 892 unique sources. Production remains NO_GO.
