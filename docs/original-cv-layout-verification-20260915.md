...
With the subsequent parser checkpoints documented in `cv-ocr-upload-release.md`, the unchanged 277-source export now yields 78 profiles / 311 employment rows, with 199 profiles without employment. Original files have not been written back to that export or the database; the 59 original-file rows must not be added to its counts as if backfill had occurred. The other 693 sources remain unaudited. Existing malformed, duplicate and invalid-range diagnostic counts remain zero.
