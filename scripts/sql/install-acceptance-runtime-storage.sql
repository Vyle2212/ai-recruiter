...
begin
  if not exists (
    ...
    where project_ref = 'iujucosewivndjpcjbuz'
      and environment_id = 'ai-recruiter-acceptance-a0123c9'
      and classification = 'acceptance'
      and acceptance_enabled = true
  ) then
    raise exception 'Wrong environment: this installer is only for the isolated Acceptance project';
  end if;
end $$;
