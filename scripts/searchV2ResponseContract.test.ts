...
assert.equal(
  normalizeSearchV2Response(empty, { allowEmptyInitialization: true }).ok,
  true,
);
assert.equal(
  `${empty.summary.visibleTotal.toLocaleString()} candidates found`,
  "0 candidates found",
);
...
if (normalizedPageTwo.ok)
  assert.equal(
    normalizedPageTwo.response.summary.visibleTotal,
    63,
    "pagination must retain the population total, not results.length",
  );

const reconciledExternalAudit = normalizeSearchV2Response(
  response({
    provider: "exa",
    results: Array.from({ length: 20 }, (_, index) => ({
      candidateId: `external-${index}`,
      rankingScore: 26,
      overallMatchScore: 26,
      overallMatchPercent: 26,
      score: { finalScore: 26 },
    })),
    aggregation: {
      providerRecordsFetched: 200,
      recordsNormalized: 193,
      invalidRecords: 7,
      ...
      uniqueProfiles: 120,
      evidenceSupported: 0,
      needsVerification: 113,
      confirmedExclusions: 7,
      eligibleVisibleResults: 113,
      currentlyRenderedResults: 20,
      remainingLoadedResults: 93,
      additionalProviderSegmentsAvailable: true,
      lastBatch: null,
    },
    summary: {
      ...(response().summary as object),
      totalDocuments: 120,
      totalMatched: 113,
      eligibleTotal: 113,
      visibleTotal: 113,
...
assert.equal(reconciledExternalAudit.ok, true);
const unexplainedExternalLoss = response({
  provider: "exa",
  results: Array.from({ length: 20 }, (_, index) => ({
    candidateId: `external-invalid-${index}`,
    rankingScore: 26,
    overallMatchScore: 26,
    overallMatchPercent: 26,
    score: { finalScore: 26 },
...
assert.equal(
  normalizeSearchV2Response(unexplainedExternalLoss).ok,
  false,
  "seven fetched records may not disappear without an explicit pre-normalization category",
);
...
assert.equal(
  normalizeSearchV2Response(paginatedWithoutTotal).ok,
  false,
  "a real paginated response must never use results.length as its total",
);
...
for (const invalid of [undefined, null, Number.NaN, "63", -1]) {
  ...
  assert.equal(
    normalizeSearchV2Response(value).ok,
    false,
    `invalid visibleTotal ${String(invalid)} must be rejected`,
  );
}
...
assert.doesNotMatch(
  ...
  "presentation must consume the normalized contract, not hide malformed state",
);
