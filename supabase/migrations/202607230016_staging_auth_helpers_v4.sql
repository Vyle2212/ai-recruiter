-- STAGING AUTH HELPERS v4 - AUTHORITATIVE PENDING-MANUAL-REVIEW ARTIFACT
-- VERSIONED, ONE-SHOT, STAGING-ONLY, NOT RERUNNABLE, NOT EXECUTED. PRODUCTION BLOCKED.
-- V4 preflight requires current_user = postgres and exact auth.uid() compatibility.
-- SECURITY DEFINER owner is postgres by creation context. Fixed search_path; no ALTER OWNER, dynamic SQL, or secrets.
-- FORCE RLS behavior and recursion safety still require controlled real staging validation.
...
as $function$ begin new.updated_at := statement_timestamp(); return new; end; $function$;
...
returns trigger language plpgsql security invoker set search_path = pg_catalog, public
as $function$ begin raise exception using errcode = '42501', message = 'audit_log_mutation_denied'; end; $function$;
...
-- REQUIRED STAGING VALIDATION - NOT EXECUTED: function owner/grants, auth.uid context, inactive identities,
-- FORCE RLS direct-table behavior, recursion/stack behavior, protected updates, and every active role.
