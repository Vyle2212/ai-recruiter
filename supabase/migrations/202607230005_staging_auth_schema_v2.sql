-- STAGING AUTH SCHEMA v2 - AUTHORITATIVE PENDING-REVIEW ARTIFACT
-- VERSIONED, ONE-SHOT, STAGING-ONLY, NOT RERUNNABLE, NOT EXECUTED.
-- Protected by 202607230004_staging_auth_preflight.sql. Manual approval and backup required.
-- PRODUCTION BLOCKED. No candidate-domain table is created, read, copied, or modified.
-- pgcrypto must already be installed; this migration never creates extensions.
...
comment on table public.user_invites is 'Intent only. Token storage, acceptance, and email delivery remain deferred; plaintext tokens are prohibited.';
...
comment on table public.access_audit_logs is 'Application append-only. Elevated database owners can remove controls and require operational governance.';
