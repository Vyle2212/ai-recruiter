-- STAGING AUTH SCHEMA v4 - AUTHORITATIVE PENDING-MANUAL-REVIEW ARTIFACT
-- VERSIONED, ONE-SHOT, STAGING-ONLY, NOT RERUNNABLE, NOT EXECUTED. PRODUCTION BLOCKED.
-- Protected by V4 preflight. Backup and explicit approval required. No candidate-domain object is touched.
...
create table public.user_profiles (
  id uuid primary key default gen_random_uuid(), auth_user_id uuid not null,
  email text not null constraint user_profiles_email_not_blank check (btrim(email) <> ''), full_name text,
  ...
  organization_id uuid, client_id uuid, candidate_id uuid,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
...
comment on table public.user_invites is 'No plaintext token or email delivery. One pending invite per normalized email globally is intentional for the single-profile-per-email model.';
...
create table public.client_memberships (
  id uuid primary key default gen_random_uuid(), user_profile_id uuid not null, organization_id uuid not null, client_id uuid not null,
...
create table public.candidate_accounts (
  id uuid primary key default gen_random_uuid(), user_profile_id uuid not null, candidate_id uuid not null,
...
create table public.access_audit_logs (
  ...
  resource_id text, result text not null constraint access_audit_logs_result_check check (result in ('allowed','denied','blocked','failed_safe')),
...
create table public.staging_auth_bootstrap_provenance (
  id uuid primary key,
  bootstrap_reference text not null constraint staging_auth_bootstrap_reference_not_blank check (btrim(bootstrap_reference) <> ''),
  organization_id uuid not null, admin_profile_id uuid not null, auth_user_id uuid not null,
  normalized_admin_email text not null constraint staging_auth_bootstrap_email_not_blank check (btrim(normalized_admin_email) <> ''),
  bootstrap_type text not null constraint staging_auth_bootstrap_type_check check (bootstrap_type = 'initial_owner'),
  created_at timestamptz not null default now(),
  constraint staging_auth_bootstrap_reference_key unique (bootstrap_reference),
  constraint staging_auth_bootstrap_organization_id_key unique (organization_id),
  constraint staging_auth_bootstrap_admin_profile_id_key unique (admin_profile_id),
  constraint staging_auth_bootstrap_auth_user_id_key unique (auth_user_id),
  constraint staging_auth_bootstrap_type_key unique (bootstrap_type),
  constraint staging_auth_bootstrap_organization_fkey foreign key (organization_id) references public.organizations(id) on delete restrict,
  constraint staging_auth_bootstrap_admin_profile_fkey foreign key (admin_profile_id) references public.user_profiles(id) on delete restrict,
  constraint staging_auth_bootstrap_auth_user_fkey foreign key (auth_user_id) references auth.users(id) on delete restrict
);
comment on table public.staging_auth_bootstrap_provenance is 'Immutable, non-secret staging initial-owner provenance. Controlled postgres bootstrap and rollback only.';
