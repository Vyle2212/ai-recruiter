-- STAGING INITIAL OWNER BOOTSTRAP ROLLBACK v4 - REVIEW ONLY - NOT EXECUTED
...
begin
  ...
  if provenance_id_text='__STAGING_BOOTSTRAP_PROVENANCE_ID__'
    ...
    raise exception using errcode='P0001',message='staging_owner_v4_rollback_placeholder_not_replaced';
  end if;
  begin
  ...
  exception when invalid_text_representation then
    raise exception using errcode='P0001',message='staging_owner_v4_rollback_uuid_invalid';
  end;

  perform 1 from public.staging_auth_bootstrap_provenance p
  where p.id=v_provenance_id and p.bootstrap_reference=btrim(bootstrap_reference)
  ...
  if not found then raise exception using errcode='P0001',message='staging_owner_v4_rollback_provenance_mismatch'; end if;
  ...
  if not found then raise exception using errcode='P0001',message='staging_owner_v4_rollback_profile_mismatch'; end if;
  ...
  if not found then raise exception using errcode='P0001',message='staging_owner_v4_rollback_organization_mismatch'; end if;

  if exists(select 1 from public.client_memberships where user_profile_id=v_admin_profile_id or organization_id=v_organization_id)
    or exists(select 1 from public.candidate_accounts where user_profile_id=v_admin_profile_id)
    or exists(select 1 from public.access_audit_logs where actor_profile_id=v_admin_profile_id)
    or exists(select 1 from public.user_invites where organization_id=v_organization_id)
    or exists(select 1 from public.user_profiles where organization_id=v_organization_id and id<>v_admin_profile_id) then
    raise exception using errcode='P0001',message='staging_owner_v4_rollback_dependent_record_exists';
  end if;

  delete from public.staging_auth_bootstrap_provenance where id=v_provenance_id and bootstrap_reference=btrim(bootstrap_reference);
  get diagnostics affected_rows=row_count;
  if affected_rows<>1 then raise exception using errcode='P0001',message='staging_owner_v4_rollback_provenance_delete_count_invalid'; end if;
  delete from public.user_profiles where id=v_admin_profile_id and auth_user_id=v_auth_user_id;
  get diagnostics affected_rows=row_count;
  if affected_rows<>1 then raise exception using errcode='P0001',message='staging_owner_v4_rollback_profile_delete_count_invalid'; end if;
  delete from public.organizations where id=v_organization_id and btrim(name)=btrim(organization_name);
  get diagnostics affected_rows=row_count;
  if affected_rows<>1 then raise exception using errcode='P0001',message='staging_owner_v4_rollback_organization_delete_count_invalid'; end if;
end;
$rollback$;
commit;
