...
export default function ImportStagingPage() {
  ...
  return <main className="min-h-screen bg-[#05070A] text-slate-100">
    <header className="border-b border-slate-800 bg-[#070A0F] px-6 py-5"><div className="mx-auto flex max-w-[1500px] flex-wrap items-end justify-between gap-3"><div><Link href="/recruiter/workflow" className="text-sm text-cyan-100">Back to recruiter workflow</Link><div className="mt-2 flex items-center gap-2"><h1 className="text-2xl font-semibold">Data Import</h1><span className="rounded-full border border-slate-600 px-2 py-1 text-[10px] font-semibold uppercase text-slate-300">Admin / Data</span></div><p className="mt-1 text-sm text-slate-400">Stage uploads/reuploads before merge. Main DB remains unchanged.</p><p className="mt-2 text-xs text-amber-100">Admin/Data tool. Read-only preview unless an explicit approved flow is used.</p></div><div className="flex gap-2"><Link href="/recruiter/import-merge" className="rounded-md bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950">Open Merge Review</Link><button type="button" onClick={buildPreview} className="rounded-md border border-cyan-400 px-4 py-2 text-sm font-semibold text-cyan-100">Build merge preview</button><button type="button" disabled className="cursor-not-allowed rounded-md bg-slate-700 px-4 py-2 text-sm text-slate-400">Merge disabled in v1</button></div></div></header>
