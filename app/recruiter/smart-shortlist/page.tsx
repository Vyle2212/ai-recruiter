...
export default function SmartShortlistPage() {
  ...
  return <main className="min-h-screen bg-[#05070A] text-slate-100">
    <header className="border-b border-slate-800 bg-[#070A0F] px-6 py-6"><div className="mx-auto flex max-w-[1500px] flex-wrap items-end justify-between gap-4"><div><Link href="/recruiter/dashboard" className="text-sm text-cyan-100">Back to Dashboard</Link><h1 className="mt-2 text-3xl font-semibold">Smart Shortlist Board</h1><p className="mt-1 text-sm text-slate-400">Ready for shortlist only. Select 2-5 candidates for compare or a client report.</p></div>{board ? <div className="rounded-lg border border-slate-800 bg-slate-900/50 px-4 py-3 text-right text-xs text-slate-400"><span className="text-emerald-100">Read-only</span><br />{board.source} &middot; {board.workflowGeneratedAt}</div> : null}</div></header>
