...
export async function POST(req: NextRequest) {
  const authorization = await requireRecruiterApiRouteAuthorization({
    request: req,
    policyId: "workflow-actions",
  });
  if (!authorization.allowed) return authorization.response;
  try {
    const body = await req.json().catch(() => ({}));
    const audit = buildRecruiterWorkflowAuditFromReports();
    const state = audit.states.find(
      (item) => item.candidateId === body.candidateId,
    );
    if (!state)
      return NextResponse.json(
        { error: "Candidate workflow state not found" },
        { status: 404 },
      );
    return NextResponse.json({
      mode: "dry-run action preview only; no candidate DB writes",
      decision: {
        action: body.action,
        allowed: true,
        reasons: ["Report-backed workflow action preview"],
        previewOnly: true,
      },
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to preview workflow action",
      },
      { status: 500 },
    );
  }
