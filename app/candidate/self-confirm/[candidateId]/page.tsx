...
export default function CandidateSelfConfirmPage({ params }: { params: Promise<{ candidateId: string }> }) {
  ...
  return <main className="min-h-screen bg-[#05070A] text-slate-100">
    ...
    <section className="mx-auto max-w-[1000px] space-y-5 px-6 py-6">
      ...
      {profile ? <>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{["Review profile","Edit or confirm fields","Preview changes","Submit for recruiter review"].map((step,index)=><div key={step} className="rounded-lg border border-slate-800 bg-[#0B0F16] p-4"><div className="text-xs font-bold uppercase text-cyan-200">Step {index+1}</div><div className="mt-1 text-sm text-slate-200">{step}</div>{index===3?<div className="mt-1 text-xs text-slate-500">Coming soon</div>:null}</div>)}</div>
        ...
        <div className="border border-cyan-500/20 bg-cyan-500/5 p-5"><label className="flex items-start gap-3 text-sm text-slate-200"><input type="checkbox" className="mt-1" checked={Boolean(values.confirmAccuracy)} onChange={(event) => set("confirmAccuracy", event.target.checked)} /><span>I consent to submit these profile confirmations and confirm they are accurate to the best of my knowledge.</span></label><div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={previewChanges} className="rounded-md bg-cyan-400 px-5 py-2 text-sm font-semibold text-slate-950 hover:bg-cyan-300">Preview changes</button><button type="button" disabled className="cursor-not-allowed rounded-md border border-slate-700 px-5 py-2 text-sm text-slate-500">Submit confirmation  coming soon</button></div><p className="mt-2 text-xs text-slate-500">There is no final submit endpoint. Previewing does not update the candidate database.</p></div>
